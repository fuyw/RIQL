...
class Agent1:
    ...
    def actor_train_step(self,
        ...
        def loss_fn(params):
            sampled_actions = self.actor.apply({"params": params}, batch.observations)
            q1, q2 = self.critic.apply({"params": critic_params}, batch.observations, sampled_actions)
            q = jnp.minimum(q1, q2)
            actor_loss = -q
...
class Agent2:
    ...
    def actor_train_step(self,
        ...
        def loss_fn(params):
            ...
            # q = jnp.minimum(q1, q2)
            # actor_loss = -q
            q = self.critic.apply({"params": critic_params}, batch.observations, actions,
                                  method=DoubleCritic.Q1)
...
class Agent3:
    ...
    def actor_alpha_train_step(self,
        ...
        def loss_fn(params, alpha_params):
            _, sampled_actions, logp = self.actor.apply({"params": params},
                                                        rng, batch.observations)
...
class Agent4:
    def __init__(self,
        ...
        self.actor = CDC_Actor(act_dim=act_dim,
                               max_action=max_action,
                               hidden_dims=hidden_dims,
                               initializer=initializer)
        actor_params = self.actor.init(actor_key, actor_key, dummy_obs)["params"]
...
class Agent5:
    ...
    def actor_train_step(self,
                         ...
                         critic_params: FrozenDict) -> Tuple[Dict, train_state.TrainState]:
        def loss_fn(params):
            _, sampled_actions, _ = self.actor.apply({"params": params}, rng, batch.observations)
            mle_prob = self.actor.apply({"params": params}, batch.observations, batch.actions,
                                        method=self.actor.get_logprob)
            ...
            actor_loss = -self.mle_alpha*mle_prob - q
        ...
        (_, actor_info), grads = jax.value_and_grad(loss_fn, has_aux=True)(actor_state.params)
    ...
    def train_step(self,
        ...
        value_info, new_value_state = self.value_train_step(batch, value_state, critic_target_params)
        critic_info, new_critic_state = self.critic_train_step(batch, critic_state, new_value_state.params)
        actor_info, new_actor_state = self.actor_train_step(batch, rng, actor_state, critic_state.params)
        new_critic_target_params = target_update(new_critic_state.params, critic_target_params, self.tau)
        return new_actor_state, new_value_state, new_critic_state, new_critic_target_params, {
            **actor_info, **value_info, **critic_info}
