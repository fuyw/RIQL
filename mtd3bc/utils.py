...
ModelBatch = collections.namedtuple("ModelBatch", [
    "observations", "actions", "rewards", "discounts", "next_observations",
    "stds"
])
...
class ReplayBuffer:
    ...
    def add_batch(self, observations, actions, next_observations, rewards,
                  dones):
        add_num = len(actions)
...
class ModelBuffer:
    ...
    def add_batch(self,
        ...
        self.discounts[add_idx] = 1 - dones.squeeze()
        if stds is not None:
            self.stds[add_idx] = stds
        self.ptr = (self.ptr + add_num) % self.max_size
        self.size = min(self.size + add_num, self.max_size)

    def sample(self, batch_size: int) -> ModelBatch:
        idx = np.random.randint(0, self.size, size=batch_size)
        batch = ModelBatch(observations=self.observations[idx],
                           actions=self.actions[idx],
                           rewards=self.rewards[idx],
                           discounts=self.discounts[idx],
                           next_observations=self.next_observations[idx],
...
def get_training_data(replay_buffer,
                      ensemble_num=7,
                      holdout_ratio=0.1,
                      eps=1e-3):
    ...
    train_idx, target_idx = permutation[
        holdout_num:], permutation[:holdout_num]
